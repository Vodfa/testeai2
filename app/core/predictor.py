...
class MarketDirectionPredictor:
    ...
    def __init__(self, buy_threshold: float = 0.62, market_bias: float = 0.0):
    ...
    def predict(self, closes: Sequence[float], volumes: Sequence[float]) -> PredictionResult:
        ...
        probability_up = max(0.0, min(1.0, probability_up + self.market_bias))
        should_buy = probability_up >= self.buy_threshold

        details = (
            f"EMA9={ema_fast:.2f} EMA20={ema_slow:.2f} | RSI={rsi:.2f} | "
            f"VolRec={vol_mean_recent:.2f} VolBase={vol_mean_base:.2f} | Momentum={momentum:.4f} | Bias={self.market_bias:+.3f}"
        )
